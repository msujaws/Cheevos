...
exports.main = function() {
  ...
  function getCheevoListBlockText() {
    ...
    for (let i in totalCheevos) {
      ...
      h += "<ul class=awards><li class=bronze>" + cheevo.bronze + "</li>";
      h += "<li class=silver>" + cheevo.silver + "</li>";
      h += "<li class=gold>" + cheevo.gold + "</li></ul></li>";
    }
    return h;  
  }
  ...
  function onCheevosPageOpened(tab, cheevo) {
    tab.attach({
      ...
      contentScript: "populateTemplate('" + escape(cheevo.name) + "','" + escape(cheevo.message) +
                                       "','" + escape(cheevoTitle()) + "','" + escape(getCheevoListBlockText()) + "');"
    });
  }
