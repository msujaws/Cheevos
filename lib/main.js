...
exports.main = function() {
  ...
  function loadCheevosPage(cheevo) {
    tabs.open(getNotifyText(cheevo));
  }
  ...
  function onDOMContentLoaded(tab) {
    ...
    if (url.scheme == "about") {
      switch(tab.url.toLowerCase()) {
        case "about:addons":
          onURLOpened(strings.cheevos.addOnsOpened);
          break;
        case "about:robots":
          onURLOpened(strings.cheevos.robotsOpened);
          break;
        case "about:config":
          onURLOpened(strings.cheevos.configOpened);
          break;
        case "about:mozilla":
          onURLOpened(strings.cheevos.mozillaOpened);
          break;
      }
    } else {
      switch(url.host) {
        case "air.mozilla.org":
          onURLOpened(strings.cheevos.revolutionTelevised);
          break;
      }
    }
