...
exports.main = function() {
  ...
  function onObservation(cheevo, shouldShow, specificAward) {
    ...
    if (specificAward) {
      if (shouldShow) {
        switch (specificAward) {
          case "bronze":
            ...
            ss[cheevo.bronzeId] = new Date().toDateString();
            break;
          case "silver":
            checkForCheevoId(cheevo, cheevo.silverId);
            ss[cheevo.silverId] = new Date().toDateString();
            break;
          case "gold":
            checkForCheevoId(cheevo, cheevo.goldId);
            ss[cheevo.goldId] = new Date().toDateString();
            break;
          default:
      ...
      acquiredCheevos++;
      ss[cheevo.id] = new Date().toDateString();
      notify(cheevo);
    }
  }
