...
const ao = require("achievement-overlay"),
      {Cc,Ci,Cu} = require("chrome"),
      BrandStringBundle = Cc["@mozilla.org/intl/stringbundle;1"]
                            .getService(Ci.nsIStringBundleService)
                            .createBundle("chrome://branding/locale/brand.properties"),
      {data} = require("sdk/self"),
      events = require("sdk/system/events"),
      {listen} = require("xul/listen"),
      {MatchPattern} = require("sdk/util/match-pattern"),
      ss = require("sdk/simple-storage").storage,
      tabs = require("sdk/tabs"),
      {unload} = require("addon/unload"),
      urlFactory = require("sdk/url"),
      winUtils = require("sdk/deprecated/window-utils"),
      _ = require("sdk/l10n").get,
      SimplePrefs = require("sdk/simple-prefs"),
      PrivateBrowsing = require("sdk/private-browsing");

  const brandShortName = BrandStringBundle.GetStringFromName("brandShortName");
  const aboutCheevosUrl = "about:cheevos";
  const addOnAchievementObs = "cheevos.api.add-on-achievement";
  ...
  let cheevos = {
    ...
    newsJunkie: {
      id: "newsJunkie",
      obs: "newsJunkie",
      description: _("newsJunkie.description"),
      hint: _("newsJunkie.hint"),
      name: _("newsJunkie.name"),
      subAwards: true,
      bronzeId: "newsJunkieBronze",
      silverId: "newsJunkieSilver",
      goldId: "newsJunkieGold",
    },
    cheevosTheyreGreat: {
      id: "cheevosTheyreGreat",
      obs: "cheevosTheyreGreat",
      description: _("cheevosTheyreGreat.description"),
      hint: _("cheevosTheyreGreat.hint"),
      name: _("cheevosTheyreGreat.name"),
      subAwards: true,
      bronzeId: "cheevosTheyreGreatBronze",
      silverId: "cheevosTheyreGreatSilver",
      goldId: "cheevosTheyreGreatGold",
    },
  };

  if (!ss.dynamicCheevos)
    ss.dynamicCheevos = {};

  // Need to load any dynamic cheevos that were persisted.
  for (let index in ss.dynamicCheevos) {
    cheevos[index] = ss.dynamicCheevos[index];
  }
  ...
  if (!ss.geekCredVisits)
    ss.geekCredVisits = 0;
  if (!ss.newsJunkieVisits)
    ss.newsJunkieVisits = 0;
  ...
  addObs(addOnAchievementObs, onAddOnAchievement, this);
  tabs.on('ready', onDOMContentLoaded);

  let toolbarbutton = require('ui/toolbarbutton').ToolbarButton({
  ...
  function addObs(topic, callback, thisRef) {
    events.on(topic, function(data) {
      callback.call(thisRef, data.subject, data.data);
    }, true);
  }
    ...
    if (specificAward) {
      switch (specificAward) {
        ...
        case "gold":
          ...
          break;
        default:
          log("Cheevo: Unexpected observation specificAward: " + specificAward);
          break;
      }
    } else {
      if (ss.cheevosAcquired[cheevo.id])
  ...
  function onAddOnAchievement(subject, data) {
    log("onAddOnAchievement with subject: " + subject + "; and data: " + data);
    // Validate data
    if (!data)
      return;

    let [id, description, hint, name, subAwards, specificAward] =
      data.split(";");
    subAwards = subAwards == "true";
    if (!id || !description || !hint || !name)
      return;
    if (subAwards && specificAward != "bronze" &&
        specificAward != "silver" && specificAward != "gold")
      return;

    let cheevo = {
      id: id,
      obs: addOnAchievementObs,
    ...
    // Since this is a dynamic cheevo, it's not stored in totalCheevos.
    let cheevoSeenBefore = false;
    for (let i in totalCheevos) {
      if (totalCheevos[i].id == id)
        cheevoSeenBefore = true;
    }
    if (!cheevoSeenBefore) {
      totalCheevos[totalCheevos.length] = cheevo;
      totalAwards += subAwards ? 3 : 1;
    }

    // Also need to persist it so it will appear in totalCheevos
    // upon restart.
    if (!ss.dynamicCheevos[id]) {
      ss.dynamicCheevos[id] = cheevo;
    }

    onObservation(cheevo, true, specificAward);
  }
  ...
  function onDOMContentLoaded(tab) {
    ...
    if (tab.url.toLowerCase() == aboutCheevosUrl) {
      onObservation(cheevos.cheevosTheyreGreat, true, "bronze");
    } else if (tab.url.toLowerCase().contains("firefox/addon/cheevos")) {
      onObservation(cheevos.cheevosTheyreGreat, true, "silver");
    } else if (tab.url.toLowerCase().contains("cheevos.msuja.ws")) {
      onObservation(cheevos.cheevosTheyreGreat, true, "gold");
    }
    ...
    let url = urlFactory.URL(tab.url);
    for (let index in cheevos) {
    ...
    if (tab.title && tab.title.toLowerCase().contains(_("noun.news"))) {
      ss.newsJunkieVisits++;
      if (ss.newsJunkieVisits >= 3)
        onObservation(cheevos.newsJunkie, true, "bronze");
      if (ss.newsJunkieVisits >= 10)
        onObservation(cheevos.newsJunkie, true, "silver");
      if (ss.newsJunkieVisits >= 20)
        onObservation(cheevos.newsJunkie, true, "gold");
    }
  ...
  var windowDelegate = {
    onTrack: function (window) {
      ...
      // Private browsing cheevos
      if (PrivateBrowsing.isPrivate(window)) {
        ss.privateBrowsingEntrances++;
        onObservation(cheevos.privateBrowsingEnter, true, "bronze");
        if (ss.privateBrowsingEntrances > 10)
          onObservation(cheevos.privateBrowsingEnter, true, "silver");
        if (ss.privateBrowsingEntrances > 50)
          onObservation(cheevos.privateBrowsingEnter, true, "gold");
      }
  ...
  // pref observer 
  SimplePrefs.on(cheevos.homepageChanged.obs, function(name) {
...
// set up about:cheevos
require('scheme/about').add({what: 'cheevos', url: data.url("cheevo.html")});
