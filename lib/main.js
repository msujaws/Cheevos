...
exports.main = function(options) {
  ...
  function toolbarbuttonLabel(awards) {
    return acquiredAwards.toString() + (acquiredAwards == 1 ? "pt" : "pts");
  };
  ...
  log("loadReason? " + options.loadReason);
  ...
  function onObservation(cheevo, shouldShow, specificAward) {
    ...
    function acquireCheevo(cheevo, id) {
      if(!ss[id]) {
        ...
        acquiredAwards++;
        notify(cheevo);
        toolbarbutton.updateLabel(toolbarbuttonLabel(acquiredAwards));
      };
    };
    if (specificAward) {
      if (shouldShow) {
