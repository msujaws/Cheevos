exports.main = function(options) {
  ...
  let strings = {
    cheevos: {
      lwThemeChanged: {
        ...
        description: _("lwThemeChanged.description"),
        hint: _("lwThemeChanged.hint"),
        learnMoreUrl: "http://www.getpersonas.com",
        learnMoreLabel: _("lwThemeChanged.learnMoreLabel"),
      ...
      bookmarkAdded: {
        ...
        learnMoreLabel: "Learn more",
        hint: "My God, it's full of stars!",
        name: "Dog eared",
        subAwards: true,
      ...
      sociallyAwesomePenguin: {
        ...
        description: "Everybody knows everybody in your social networking site.",
        hint: "Where is the dislike button?",
        name: "6deg of separation",
        subAwards: true,
      ...
      socialButterfly: {
        id: "socialButterfly",
        obs: "socialButterfly",
        description: "Firefox is the best tool for checking in on your social networks.",
        hint: "Can you retweet my post on FaceSpace?",
      ...
      tabCandy: {
        id: "tabCandy",
        obs: "tabCandy",
        description: "Tabs provide a great user interface for viewing multiple websites at the same time.",
        hint: "Oooh, tabs are teh pretty",
        name: "Tab Candy",
        subAwards: true,
      ...
      concurrentTabs: {
        ...
        obs: "concurrentTabs",
        description: "Tabs provide a great user interface for viewing multiple websites at the same time.",
        hint: "Whoa, so many tabs dude!",
        name: "Concurrent Tabs",
        subAwards: true,
      ...
      geekCred: {
        id: "geekCred",
        obs: "geekCred",
        description: "Online communities are a great way to find people with similar interests.",
        hint: "So I was browsing today and I found some upvotes...",
        name: "Geek Cred",
        subAwards: true,
  ...
  function onObservation(cheevo, shouldShow, specificAward) {
    ...
    function acquireCheevo(cheevo, id, points, awardsClass) {
      if (!ss.cheevosAcquired[id]) {
        ...
        toolbarbutton.updateLabel(acquiredPoints);
      };
    };
    if (!shouldShow)  
      return;
    ...
    for (var i = 0; i < socialSites.length; i++) {
      ...
      if (new MatchPattern(socialSite).test(tab.url)) {
        ...
        if (ss.socialVisits >= 3)
          onObservation(strings.cheevos.socialButterfly, true, "bronze");
        if (ss.socialVisits >= 10)
          onObservation(strings.cheevos.socialButterfly, true, "silver");
        if (ss.socialVisits >= 20)
          onObservation(strings.cheevos.socialButterfly, true, "gold");
        if (socialSite in ss.specificSocialSiteVisits) {
          ...
          if (specificSocialSiteVisit >= 5 && specificSocialSiteVisit != ss.socialVisits)
            onObservation(strings.cheevos.sociallyAwesomePenguin, true, "bronze");
          if (specificSocialSiteVisit >= 15 && specificSocialSiteVisit != ss.socialVisits)
            onObservation(strings.cheevos.sociallyAwesomePenguin, true, "silver");
          if (specificSocialSiteVisit >= 25 && specificSocialSiteVisit != ss.socialVisits)
            onObservation(strings.cheevos.sociallyAwesomePenguin, true, "gold");
        } else {
    ...
    for (var i = 0; i < geekCredSites.length; i++) {
      ...
      if (new MatchPattern(geekCredSite).test(tab.url)) {
        ...
        if (ss.geekCredVisits >= 3)
          onObservation(strings.cheevos.geekCred, true, "bronze");
        if (ss.geekCredVisits >= 10)
          onObservation(strings.cheevos.geekCred, true, "silver");
        if (ss.geekCredVisits >= 20)
          onObservation(strings.cheevos.geekCred, true, "gold");
        break;
      }
    }
  ...
  function tabAdded() {
    ss.tabsCount++;
    if (ss.tabsCount >= 5)
      onObservation(strings.cheevos.tabCandy, true, "bronze");
    if (ss.tabsCount >= 50)
      onObservation(strings.cheevos.tabCandy, true, "silver");
    if (ss.tabsCount >= 500)
      onObservation(strings.cheevos.tabCandy, true, "gold");

    let concurrentTabs = tabs.length;
    if (concurrentTabs >= 5)
      onObservation(strings.cheevos.concurrentTabs, true, "bronze");
    if (concurrentTabs >= 25)
      onObservation(strings.cheevos.concurrentTabs, true, "silver");
    if (concurrentTabs >= 100)
      onObservation(strings.cheevos.concurrentTabs, true, "gold");
  }
