...
exports.main = function(options) {
  ...
  function onObservation(cheevo, shouldShow, specificAward) {
    ...
    function acquireCheevo(cheevo, id, points) {
      if (!ss[id]) {
        ss[id] = new Date().toDateString();
        acquiredAwards++;
        notify(cheevo, points);
    ...
    if (specificAward) {
      switch (specificAward) {
        case "bronze":
          if (ss[cheevo.bronzeId])
            return;
          acquiredPoints += PTS_PER_BRONZE;
          acquireCheevo(cheevo, cheevo.bronzeId, PTS_PER_BRONZE);
          break;
        case "silver":
          if (ss[cheevo.silverId])
            return;
          acquiredPoints += PTS_PER_SILVER;
          acquireCheevo(cheevo, cheevo.silverId, PTS_PER_SILVER);
          break;
        case "gold":
          if (ss[cheevo.goldId])
            return;
          acquiredPoints += PTS_PER_GOLD;
          acquireCheevo(cheevo, cheevo.goldId, PTS_PER_GOLD);
          break;
        default:
      ...
      if (ss[cheevo.bronzeId])
        return;
      acquiredPoints += PTS_PER_BASIC;
      acquireCheevo(cheevo, cheevo.id, PTS_PER_BASIC);
    }
  }
